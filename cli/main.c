...
static void print_usage(const char *prog)
{
    printf(
        ...
        "\n"
        "Modes:\n"
        "  --status                      Dump chip ID, status and error registers\n"
        "  --rx                          Continuous receive (LoRa or FSK)\n"
        "  --tx                          Transmit packets (LoRa or FSK)\n"
        "  --scan                        Fast CAD frequency scan\n"
        "  --txtest                      RF TX test: CW or infinite preamble (spectrum analyser)\n"
        "  --mavlink                     MAVLink telemetry bridge over 2.4 GHz LoRa\n"
        "\n"
        "SPI / board:\n"
        "  --spi <spi0|spi1>             SPI bus (default: spi0)\n"
        "\n"
        "FSK modem (--rx / --tx / --txtest):\n"
        "  --fsk                         Use GFSK instead of LoRa\n"
        "  --bitrate <bps>               FSK bitrate (default: 50000)\n"
        "  --fdev <Hz>                   FSK frequency deviation (default: 25000)\n"
        "  (Sync: 4-byte 0x2DD42DD4, GFSK BT=0.5, 2-byte CRC, whitening on)\n"
        "\n"
        "TX test options (--txtest):\n"
        "  --cw                          Unmodulated CW carrier (default: infinite preamble)\n"
        "  (use --freq, --power, --hp-pa, --sf, --bw, --fsk as for rx/tx)\n"
        "\n"
        "LoRa modulation (rx / tx / scan / mavlink):\n"
        "  --freq <Hz>                   Centre frequency in Hz (default: 868100000)\n"
        "  --sf <5-12>                   Spreading factor (default: 7)\n"
        "  --bw <kHz>                    Bandwidth: 125/250/500 (sub-GHz) or 200/400/800 (2.4G) (default: 125)\n"
        "  --cr <5-8>                    Coding rate denominator 4/x (default: 5)\n"
        "  --preamble <symbols>          Preamble length (default: 8)\n"
        "  --no-crc                      Disable CRC\n"
        "  --implicit                    Implicit header mode\n"
        "  --payload-len <bytes>         Expected payload length (implicit only, default: 255)\n"
        "  --ldro                        Enable low data rate optimisation\n"
        "  --invert-iq                   Invert IQ\n"
        "  --sync-word <hex>             LoRa sync word byte (default: 0x12 private)\n"
        "                                  0x12 = private / satellite LoRa\n"
        "                                  0x34 = LoRaWAN public\n"
        "                                  0x2B = Meshtastic\n"
        "\n"
        "RX options:\n"
        "  --bw-boosted                  Enable boosted LNA (sub-GHz)\n"
        "\n"
        "TX options:\n"
        "  --power <dBm>                 TX power in dBm (default: 14)\n"
        "  --hp-pa                       Use HP PA (sub-GHz >15 dBm)\n"
        "  --payload <string>            ASCII payload string\n"
        "  --payload-hex <hexstring>     Hex payload (e.g. deadbeef)\n"
        "  --count <n>                   Packets to send, 0=infinite (default: 1)\n"
        "  --interval <ms>               Inter-packet delay ms (default: 1000)\n"
        "\n"
        "Scan options:\n"
        "  --freq-start <Hz>             Scan start (default: 863000000)\n"
        "  --freq-end   <Hz>             Scan end   (default: 870000000)\n"
        "  --freq-step  <Hz>             Step size  (default: 200000)\n"
        "  --cad-symb <1|2|4|8|16>      CAD symbol count (default: 1)\n"
        ...
        "\n"
        "MAVLink bridge options (--mavlink):\n"
        "  --air                         This unit is on the drone (TDMA master/initiator)\n"
        "                                  Omit for ground side (TDMA slave/responder)\n"
        "  --uart <path>                 Hardware UART device (air side, e.g. /dev/serial0)\n"
        "  --baud <rate>                 UART baud rate (default: 57600)\n"
        "  --tcp-port <port>             TCP server port (ground side, e.g. 5760)\n"
        "  --pty <path>                  PTY symlink for testing (e.g. /tmp/ttyLORA0)\n"
        "  --slot-ms <ms>                TDMA slot interval in ms (default: 200)\n"
        "                                  Must be > air TX time + guard + GND TX time\n"
        "                                  SF7/BW800 max packet ~62 ms; 200 ms gives 5 Hz\n"
        "  --guard-ms <ms>               Guard time GND waits before responding (default: 10)\n"
        "  (Defaults: freq=2440000000, SF7, BW800, CR4/5, power=13 dBm)\n"
        "\n"
        "Examples:\n"
        "  %s --spi spi0 --status\n"
        "  %s --spi spi0 --rx --freq 868300000\n"
        "  %s --spi spi0 --rx --freq 868300000 --sf 9 --bw 125\n"
        "  %s --spi spi0 --rx --freq 2445000000 --sf 7 --bw 400\n"
        "  %s --spi spi0 --tx --freq 868300000 --payload \"hello\" --count 5\n"
        "  %s --spi spi0 --scan\n"
        "  %s --spi spi0 --scan --freq-start 863000000 --freq-end 928000000 \\\n"
        "                       --freq-step 100000 --sf 7 --bw 125 --continuous\n"
        "  %s --spi spi0 --mavlink --air --freq 2440000000 --uart /dev/serial0 --baud 57600\n"
